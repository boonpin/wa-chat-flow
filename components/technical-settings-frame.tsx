...
/** Technical subsection navigation stays inside the content container. */
export function TechnicalSettingsFrame({ children }: { children: ReactNode }) {
  const pathname = usePathname()
  if (getSettingsSection(pathname)?.key !== 'technical') return children
  const current = getTechnicalSection(pathname)
  return (
    <div className="grid max-w-[1440px] gap-5 p-4 md:p-6 lg:grid-cols-[200px_minmax(0,1fr)] lg:gap-6">
      <aside className="min-w-0 border-b border-line pb-4 lg:border-r lg:border-b-0 lg:pr-4 lg:pb-0">
        <p className="mb-3 text-xs font-semibold text-ink-muted">Technical settings</p>
        <nav aria-label="Technical settings sections">
          <ul className="grid grid-cols-2 gap-1 lg:grid-cols-1">
            {TECHNICAL_SECTIONS.map((item) => (
              <li key={item.key}>
                <Link
                  href={item.href}
